---
title: "PASS00589 DDA mspip (large k)"
...
```{r include=TRUE}
dda_pip <- mspip("/stornext/General/data/academic/lab_davis/prot/benchmarking/PASS00589/Standard-Profiling-Sample-Set/combined/txt", 
                 k=2, thresh = 0.5, tims_ms = FALSE, skip_weights = TRUE)
...
dda_pip2 <- mspip("/stornext/General/data/academic/lab_davis/prot/benchmarking/PASS00589/Standard-Profiling-Sample-Set/combined/txt",
                  k=2, thresh = 0.0, tims_ms = FALSE, skip_weights = FALSE)
...
return_EList <- TRUE
evidence <- dda_pip
y2 <- aggregate(evidence[,"Intensity"] ~ evidence[, run_id] + evidence[, peptide_id],
                 FUN = function(x) max(x, na.rm=TRUE),
                 na.action = na.pass)

  colnames(y2) <- c(run_id, peptide_id, "Intensity")
  y2[y2==-Inf] <- NA

  E <- tidyr::spread(y2, key = 1, value = 3)
  ...
  if(return_EList){
    ...
    idx <- match(paste0(y2[,run_id], y2[,peptide_id], y2[,"Intensity"]),
...
# y2 <- evidenceToMatrix(data, return_EList = TRUE, weights = "weight")
# y3 <- evidenceToMatrix(dda_pip2, return_EList = TRUE, weights = "weight")
```
...
mean(is.na(y_pip))
mean(is.na(y_pip_elist$E))
mean(is.na(y))

colMeans(is.na(y_pip))
colMeans(is.na(y_pip_elist$E))
colMeans(is.na(y))

table(rowSums(!is.na(y_pip)) >=  4)
table(rowSums(!is.na(y)) >=  4)


par(mfrow=c(1,2))
pcv <- plotCV2(log2(y_pip), main = "PIP")
pcv <- plotCV2(log2(y), main = "data")

barplot(apply(log2(y),2,min, na.rm=TRUE))
barplot(apply(log2(y_pip),2,min, na.rm=TRUE))
```
...
volcanoplot(fit1, coef = 8)
plotSA(fit1)


fit2 <- lmFit(y_pip, design)
fit2 <- eBayes(fit2)
summary(decideTests(fit2))
volcanoplot(fit2, coef = 8)
plotSA(fit2)

fit3 <- lmFit(y_pip_elist, design)
fit3 <- eBayes(fit3)
summary(decideTests(fit3))
volcanoplot(fit3, coef = 2)
plotSA(fit3)
...
Make a plot for github README
```{r include=TRUE, out.width="90%", eval = FALSE}
par(mfrow = c(2,2))
pcv <- plotCV2(y, main = "data", ylim=c(0,0.05))
pcv <- plotCV2(y_pip, main = "PIP",ylim=c(0,0.05))
volcanoplot(fit1, coef = 8)
volcanoplot(fit2, coef = 8)
```
...
## DIA dataset
```{r eval = FALSE}
dia <- read.csv("/stornext/General/data/academic/lab_davis/prot/benchmarking/PASS00589/Bruderer2015_DIA_Skyline_input.csv", stringsAsFactors = FALSE)

dia$Area <- as.numeric(dia$Area)
dia$DetectionQValue <- as.numeric(dia$DetectionQValue)

table(dia$DetectionQValue < 0.01)
table(is.na(dia$DetectionQValue))


table(grepl("CON__|REV__", dia$PeptideModifiedSequence))

keep1 <- (!is.na(dia$DetectionQValue) & dia$DetectionQValue < 0.01)
table(keep1)



# remove peptides with qvalue > 0.1 (10% FDR) (although this may be too stringent)
dia <- dia[keep1,]
dia$PeptideId <- paste(dia$PeptideModifiedSequence, dia$PrecursorCharge, dia$ProteinName, sep="_")




# first sum peptides per replicates, then take median over all replicates
y_dia <- aggregate(Area ~ FileName + PeptideId, 
                    FUN=function(x) max(x, na.rm = TRUE),
               # FUN = function(x) median(x, na.rm = TRUE),
               na.action = na.pass, data = dia)




y_dia <- spread(y_dia, key = FileName, value = Area)
rownames(y_dia) <- y_dia[,1]
y_dia <- y_dia[,-1]

dim(y_dia)

table(complete.cases(y_dia))
table(rowSums(y_dia == 0) < 6)

y_dia[y_dia==0] <- NA


keep <- rowSums(!is.na(y_dia)) >= 4
table(keep)

# keep2 <- rowSums(y_dia == 0) < 6


y_dia <- y_dia[keep,]
y_dia <- log2(y_dia)

colnames(y_dia) <- gsub("_MHRM_T0.raw","_MSG", colnames(y_dia))
all(colnames(y_dia) == colnames(y_dda))
```
